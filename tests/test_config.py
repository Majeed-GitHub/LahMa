...
def mock_config_files(tmp_path):
    ...
    invalid_yaml_path.write_text("logging: level: DEBUG\nlogging: level: INFO") # Duplicate key
...
def test_find_config_specific_path_exists(mock_config_files):
    ...
    assert found_path == str(mock_config_files["specific"]) # Should return the same absolute path
...
def test_find_config_default_path_exists(mock_config_files):
    ...
    os.chdir(mock_config_files["tmp_path"])
    expected_relative_default = os.path.join(CONFIG_DIR, "config.yaml") # Expect relative path
...
def test_find_config_fallback_to_example(mock_config_files):
    ...
    expected_relative_example = os.path.join(CONFIG_DIR, "config.example.yaml") # Expect relative path
