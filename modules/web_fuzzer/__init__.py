...
try:
     ...
     print("ERROR: Cannot import core exceptions from web_fuzzer.")
     class WebFuzzerError(Exception): pass
...
def check_nuclei_installed() -> bool:
    ...
    try:
        ...
        subprocess.run(["nuclei", "-version"], capture_output=True, text=True, check=True, timeout=10)
        logger.debug("Nuclei installation confirmed.")
        return True
    except FileNotFoundError:
        logger.error("Nuclei command not found. Please install Nuclei and ensure it's in your system PATH.")
        return False
    except subprocess.CalledProcessError as e:
        logger.warning(f"Nuclei command found, but '-version' exited with error: {e.stderr}")
        return True
...
def run_nuclei(target_url: str, templates_path: Optional[str], extra_flags_str: str, process_timeout: int) -> bool:
    """
    ...
    if extra_flags_str:
        try:
            ...
            logger.info(f"Adding extra Nuclei flags: {extra_args}")
             ...
             logger.error(f"Error parsing extra flags '{extra_flags_str}': {e}. Flags ignored.")
    ...
    try:
        ...
        if process.returncode != 0:
            ...
            raise WebFuzzerError(f"Nuclei process failed with exit code {process.returncode}.")
        else:
            logger.info("Nuclei scan process completed successfully (exit code 0).")
            return True

    except FileNotFoundError:
        logger.critical("Nuclei command failed with FileNotFoundError during execution attempt.")
        raise EnvironmentError("Nuclei executable not found during execution. Please ensure it is installed and in the system PATH.") from None
    except subprocess.TimeoutExpired:
         logger.error(f"Nuclei scan timed out after {process_timeout} seconds.")
         raise WebFuzzerError(f"Nuclei scan process timed out ({process_timeout}s).") from None
    except Exception as e:
        logger.critical(f"An unexpected error occurred while running Nuclei command: {e}", exc_info=True)
...
def run(config: Dict[str, Any]):
    ...
    if not target_url:
        raise ConfigError("Required setting 'web_fuzzer.target_url' is missing in configuration.")

    logger.info(f"Target URL: {target_url}")

    # 1. Check for external dependencies (Nuclei)
    if not check_nuclei_installed():
         raise EnvironmentError("Nuclei dependency check failed. Cannot proceed with Nuclei scan.")

    # 2. Run ParamSpider (Placeholder)
    # ...

    # 3. Run Nuclei Scan (Removed the incorrect try block here)
    logger.info("Proceeding with Nuclei scan...")
    nuclei_success = run_nuclei( # Directly call run_nuclei; it will raise errors on failure
